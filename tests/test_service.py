...
from tools.rag.generator import UNKNOWN_ANSWER
...
def test_answer_success(
    ...
    assert response.capability == "rag"
    mock_generate.assert_called_once_with("How do returns work?", [sample_chunk])
...
def test_answer_uses_rewritten_question_for_retrieval(
    ...
    mock_generate.assert_called_once_with("returns?", [sample_chunk])
